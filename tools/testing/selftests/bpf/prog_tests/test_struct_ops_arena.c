...
#include "struct_ops_arena_fail__deref_null.skel.h"
...
void serial_test_struct_ops_arena(void)
{
	 ...
	 * arm64. Elsewhere verification fails with "JIT does not support
	...
	if (test__start_subtest("arena_arg_fail"))
		arena_arg_fail();
	RUN_TESTS(struct_ops_arena_fail__deref_null);
#else
	test__skip();
#endif
}
