 ...
 * @cpu_to_cid__arena_nullable: arena array of nr_cpu_ids s32 entries (cid for
 * each cpu)
 ...
 */
__bpf_kfunc void scx_bpf_cid_override(const s32 *cpu_to_cid__arena_nullable, u32 cnt,
				      const struct bpf_prog_aux *aux)
{
	...
	/*
	 * @cpu_to_cid__arena_nullable arrives rebased to the arena kernel mapping.
	...
	for_each_possible_cpu(cpu) {
		s32 c = cpu_to_cid__arena_nullable[cpu];
